    ...
    public class IntUndirectedMap : IMap<int>
    {
        ...
        public void ConnectNode(Node<int> firstNode, Node<int> secondNode, int val)
        {
            ...
            _connectionsValues.Add(
                ...
                {
                    FirstIndex = _nodes.First(n => n == firstNode).Id,
                    SecondIndex = _nodes.First(n => n == secondNode).Id,
                    Value = val
                });
        }
        ...
        {
            if (node == null || !_nodes.Contains(node)) return;
            foreach (var connection in _connectionsValues.Where(c => c.FirstIndex == _nodes.First(n => n == node).Id))
            {
                _connectionsValues.Remove(connection);
            }
            foreach (var connection in _connectionsValues.Where(c => c.SecondIndex == _nodes.First(n => n == node).Id))
            {
                _connectionsValues.Remove(connection);
            }
